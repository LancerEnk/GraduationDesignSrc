...
async function GetOneline3(geohash) {
	...
	console.log("GetOneline3() - roads:", roads);
	ChangeJSON(roads);
}
...
async function GetOneline4(geohash) {
	...
	console.log("GetOneline4() - roads:", roads);
	ChangeJSON(roads);
}
...
async function GetOneline5(geohash) {
	...
	console.log("GetOneline5() - roads:", roads);
	ChangeJSON(roads);
}
...
async function getTypes(geohash, roads) {
	await mapContract.methods.get_types(web3Map.utils.asciiToHex(geohash)).call(function (error, result) {
		...
		if (!error) {
			...
			console.log("getTypes() - the roadname is : ",roadName);
			...
			for (var i = 0; i < roadName.length; i++) {
				console.log("getTypes() - roadinfoCost: ", roadInfo[i * 7 + 2]);
				var road = new Object();
